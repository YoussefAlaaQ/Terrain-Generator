...
void main(){
    ...
    // vec3 lightColor = vec3(1.0); // gamma correction 
    vec3 lightColor = vec3(1.0f, 0.94f, 0.82f);
    // vec3 lightColor = vec3(20.0); // no gamma correction
    
    float textureTileScale = 0.2; 
    ...
    // vec3 F  = fresnelSchlick(max(dot(h, worldViewDir), 0.0), F0); //commented this to avoid the weired artifact 
    ...
    //Distance fog
    float fogDensity = 0.0006;
    float distanceToCamera = length(viewPos_out - FragPos);
    float fogFactor = 1.0 - exp(-distanceToCamera * fogDensity);
    vec3 fogColor = vec3(0.82, 0.83, 0.80);
